...
bb >> darkpass(ch, dur=PSum(3,2), oct=(4,5), amp=.7)

vv >> darkpass(ch + [[2,0,0],0,[0,2,2],[0,0,2]], dur=ch.dur, oct=4) + var([(0,2),(2,4)],[28,4,16,16])


vv >> marimba(ch + [[2,0,0],0,[0,2,2],[0,0,2]], dur=ch.dur,
            oct=4) + var([(0,2),(2,4)],[28,4,16,16])

vv.amp=var([.7,0],[48,16])

A = var([1,0],[16.5,15.5])
zz >> zap([3,2,0], dur=1/3, sus=1/2, amp=A*2, pan=1) + [0,[0,(0,2),0],0]
pp >> vibra2([0,2,4,7,8,2], dur=1/2, sus=1/2, amp=A, pan=-1) + [0,0,[0,0,1]]
...
fi >> play("ss[ss][ss]", dur=clave23, sample=5)
