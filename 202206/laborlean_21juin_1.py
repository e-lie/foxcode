...
f1 >> fordrip(0, dur=PRand(1,4), oct=PWalk(7,1,3)[:17]).fadein()
f1.fadein()

m2 >> bass(P[.1,.2,.3,.4]*3, dur=PWhite(.1,.8), pan=[-1, 0, 1], amp=1.5, oct=6).humz()
...
d1 >> play("[i*]", amp=linvar([0,1,0], [2, 0, 2]), rate=2, crush=8, pan=[-1, 1]).fadein()

k2 >> play("Vv V v", dur=clave23)
k1 >> play("--t", dur=clave23)
k1 >> play("--t", dur=cascara)

m1.every(16, "reverse").only()
...
bb >> blip(P[0,2,-3,4].stutter(8), dur=.25, sus=sinvar([.25,2],16), oct=(4,6), pan=P[-1,0,1].stutter(2))
...
u oct=3, pan=sinvar([-.5, .5], 24)).fadein()
...
d1 >> play(".*[**].", rate=var([1,2]))
# d1 >> play(".*[.*].", rate=PWhite(.3, 3)[:17], room=[0,1,4])
...
d2 >> play("<V.O..Vo.>", dur=.5, sample=0, rate=linvar([.8,1.2], 16), amp=.8).fadein()
