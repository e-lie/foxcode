...
k1 >> kicker([1,15,7], dur=1, oct=3, amp=var([1,0],[12,4])).every(8, "stutter", 3)

s1 >> strings(base1, dur=2, oct=6, root=root1, scale=Scale.minor, vol=1, amp=var([1,0], [12,4]), i_release=.8 )
...
ramp = linvar([0,1],[30,inf],start=Clock.mod(4))
ramp2 = linvar([.9,.3,1],[2,28,inf],start=Clock.mod(4))
...
moct = var([4,5], 16) # var doesn't with ableton instruments -> bug to fix
p1 >> bell(Pvar([Scale.minor, Scale.lydian, Scale.phrygian],8), oct=moct, dur=gnawa, root=0, amp=var([.2,0,.2],[16,4,12])).every(7,"stutter",3)
p2 >> bell([P+(0,2,4,6)], dur=4, sus=4, oct=moct, root=var([0,5,4],8), scale=Scale.major, amp=var([.2,0,.2],[12,4,16]))
...
b2 >> ubass(base2, dur=PSum(5,3), amp=1, oct=3.5, root=root2, scale=scale2).stop()
...
p3 >> kitdatai(drum, dur=1/2, amp=var([1,0],[12,4]))#var([0,1,2,3],[8,8,4,5]), dur=1/2)
