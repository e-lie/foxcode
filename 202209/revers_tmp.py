...
m1 >> marimba("ab[d][cc]a.........b[aaaa]([c]a[cccc]d)[ff]", oct=[4,5], amp=sinvar([.1,.9],32), dur=.125).fadein()
m1.span(srot(24))
m1.shuffle()
m2.shuffle()

m2 >> marimba("ab[d][cc]a.b[aaaa]([cccc]d)[ff]", oct=[4,6], amp=sinvar([.1,.9],17), dur=.125, ).humz()
m2.span(srot(16))
...
tt >> play("< -><x >", amp=2).mpan(range(6))
t2 >> play("<x >").mpan(range(6)).stop()
tt.dur = cascara
t2.dur = cascara
tt.rate = 1
tt.rate = 2
...
m3 >> marimba(M + [[2, 0, 0], 0, [2, 2]], dur=cascara, oct=P[5,4,7].stutter(4))
m3.amplify = pa32_3
...
d4 >> play(".*[**].", rate=var([1,2]), amp=1.5)
d4.mpan(mrot(16))

b1.dur=cascara
b1.dur=.25

k1 >> play("V", dur=clave23, sample=P[0,1,2].stutter(5), amplify=pa32, amp=2)
...
d1 >> play("<(-~)[~-]>", dur=.5, amplify=pa16_2, amp=2)
...
v1 >> marimba(chords + [[2,0,0],0,[0,2,2],[0,0,2]], dur=chords.dur, oct=(4,5), amplify=pa32_3)
...
v2 >> marimba([0,2,4,7,8,2], dur=1/2, sus=1/2, amplify=pa16_2, pan=var([-1, 1], 48), oct=[5,6,7]) + [0,0,[0,0,1]]
